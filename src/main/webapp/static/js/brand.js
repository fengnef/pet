$(function () {
    $("#brand_datagrid").datagrid({
        ...
        columns:[[
            {field:'brandoriginname',title:'品牌产地名称',width:100,align:'center'},
            {field:'brandname',title:'商品品牌名称',width:100,align:'center'},
            {field:'state',title:'是否启用',width:100,align:'center',formatter:stateFormatter},
            {field:'operate',title:'操作',width:100,align:'center',
                formatter:function (value,row,index) {
                    return "<a name='opera' id='brand_tb_edit' onclick='edit()'></a> "+
                    "<a name='oper' id='brand_tb_cs' onclick='dlt()'></a> ";
            }}
...
function add() {
    ...
    //设置标题
    $("#brand_dialog").dialog("setTitle","新增品牌");
}
//编辑
function edit() {
    //判断是否选中数据
    ...
    $("#brand_dialog").dialog("open");
    //设置标题
    $("#brand_dialog").dialog("setTitle","编辑");
}
//删除
function dlt() {
    //判断是否选中数据
    ...
    $.messager.confirm('提示','确定删除该数据吗',function (r) {
        if (r) {
            $.post("/brand_delete.do",{id:row.id},function (data) {
                if (data.success) {
